...
async function getWeatherInfo(object) {
    await fetch(object.url)
        ...
        .then(json => {
            ...
            let arr = getInfoFor5Day(lon, lat);
            cityInfo.arrMyValues = arr;

            cityInfo.renderIn(div);
            //"http://api.openweathermap.org/data/2.5/onecall?lat=50.4333&lon=30.5167&exclude=current, minutely,hourly,alerts&appid=bf35cac91880cb98375230fb443a116f"
...
function getInfoFor5Day(lon, lat) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        let temp = json.daily[i].temp.day;
        let icon = getImgURL(json.daily[i].weather[0].icon);

        let strDayField = 'Day' + (i + 1);
        let strTempField = 'temperature' + (i + 1);
        let strImgField = 'img' + (i + 1);
        ...
        //let html = Mustache.render(document.getElementById("block").innerHTML, obj);
        //return htm
    ...
    //             let day = tom.toLocaleDateString();
    //             let temp = json.daily[i].temp;
    //             let icon = getImgURL(json.weather[0].icon);
